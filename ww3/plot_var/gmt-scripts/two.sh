...
then	
	...
	psscale -D`echo "($Xmax - $Xmin)*$mapRatioSize + 1" | bc -l`/`echo "(($Ymax - $Ymin)*$mapRatioSize)/2" | bc -l`/`echo "($Ymax - $Ymin)*$mapRatioSize" | bc -l`/`echo "(($Ymax - $Ymin)*$mapRatioSize)/40" | bc -l` -C$twoPalFile -B0.001:"":/:"": -E -O -K >> ${outfile}.ps	
	echo "5 9 12 0 5 BC Two (m 2 s -2)" > ${workingDir}/legend	

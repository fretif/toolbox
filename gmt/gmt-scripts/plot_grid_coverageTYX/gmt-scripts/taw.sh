...
then	
	...
	if [[ -n "$maskFile" ]] 
	then	
	  grdmath  ${infile}?utaw[$tIndex] $maskFile OR = ${workingDir}/u.grd
	  grdmath  ${infile}?vtaw[$tIndex] $maskFile OR = ${workingDir}/v.grd
	else
	  grdsample ${infile}?utaw[$tIndex] -G${workingDir}/u.grd
	  grdsample ${infile}?vtaw[$tIndex] -G${workingDir}/v.grd
	fi
	...
	then		
		...
		#cp ${workingDir}/colorPal.cpt taw.cpt		
	fi
	...
	gmtset MAP_TICK_PEN_PRIMARY thinner,white,solid
 	...
 	#psscale -Dx14c/12.25c+w12c/0.5c+jTC+e -C$tawPalFile -Bx0.0002+g0.0002 -By+lN/m2 -S -O -K >> ${outfile}.ps	
 	psscale -Dx19.5c/7.25c+w6c/0.5c+jTC+e -C$tawPalFile -Bx0.0002+g0.0002 -By+lN/m2 -S -O -K >> ${outfile}.ps	
	echo "5 9 20 0 5 BC TAW (N/m2)" > ${workingDir}/legend

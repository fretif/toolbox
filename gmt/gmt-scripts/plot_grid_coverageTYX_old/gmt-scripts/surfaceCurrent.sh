...
then	
	...
	if [[ -n "$maskFile" ]] 
	then	
	  grdmath  ${infile}?ucur[$tIndex] $maskFile OR = ${workingDir}/u.grd
	  grdmath  ${infile}?vcur[$tIndex] $maskFile OR = ${workingDir}/v.grd
	else
	  grdsample ${infile}?ucur[$tIndex] -G${workingDir}/u.grd
	  grdsample ${infile}?vcur[$tIndex] -G${workingDir}/v.grd
	fi
	...
	gmtset MAP_TICK_PEN_PRIMARY thinner,white,solid
 	...
 	psscale -Dx14c/12.25c+w12c/0.5c+jTC+e -C$currentPalFile -Bx0.3+g0.3 -By+lm/s -S -O >> ${outfile}.ps	
	echo "5 9 20 0 5 BC Surface current (m/s)" > ${workingDir}/legend

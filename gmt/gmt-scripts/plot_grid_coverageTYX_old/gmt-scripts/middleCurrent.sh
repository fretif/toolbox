...
then	
	...
	if [[ ! -n "$currentPalFile" ]] 
	then		
		grd2cpt -C/data/gmt-toolbox/color-palettes/pasadena.cpt ${workingDir}/plot.grd -Z > ${workingDir}/colorPal.cpt
		currentPalFile="${workingDir}/colorPal.cpt"
		#cp ${workingDir}/colorPal.cpt middle-current.cpt
		#cp ${workingDir}/plot.grd .		
	fi
	...
	grdvector ${workingDir}/current_u_light.grd ${workingDir}/current_v_light.grd  -J -R -Gblack -S${vectorLength}i -Q0.05i+e -O -K >> ${outfile}.ps
	...
	echo "5 9 12 0 5 BC -20m current (m/s)" > ${workingDir}/legend	

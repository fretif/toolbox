...
then	
	...
	then		
		...
		ibPalFile="${workingDir}/colorPal.cpt"	
	...
	# Plotting
	#	
	grdimage ${infile}?z[$tIndex] $envelope $projection -C$ibPalFile -P -K > ${outfile}.ps
	...
	psscale -D`echo "($Xmax - $Xmin)*$mapRatioSize + 1" | bc -l`/`echo "(($Ymax - $Ymin)*$mapRatioSize)/2" | bc -l`/`echo "($Ymax - $Ymin)*$mapRatioSize" | bc -l`/`echo "(($Ymax - $Ymin)*$mapRatioSize)/40" | bc -l` -C$ibPalFile -B0.1:"":/:"": -E -O -K >> ${outfile}.ps	
	echo "5 9 12 0 5 BC Inverse barometer (m)" > ${workingDir}/legend

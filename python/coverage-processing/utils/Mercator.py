...
from coverage.operator.Interpolator import resample2dToGrid
import tempfile

def resampleTXY(coverage):
    ...
    xres_mean=((Xmax-Xmin)/coverage.get_x_size())
    yres_mean=((Ymax-Ymin)/coverage.get_y_size())
    ...
    tempFilename = tempfile.TemporaryFile().name

    ncfile = Dataset(tempFilename, 'w', format='NETCDF4')
    ...
    for time in coverage.read_axis_t():  
        print np.shape(resample2dToGrid(coverage.read_axis_x(),coverage.read_axis_y(),lon_reg,lat_reg,coverage.read_variable_wlv_at_time(time)))
        wlv[time_index:time_index+1,:,:] = resample2dToGrid(coverage.read_axis_x(),coverage.read_axis_y(),lon_reg,lat_reg,coverage.read_variable_wlv_at_time(time))
        time_index += 1
    ...
    time_index=0
    for time in coverage.read_axis_t(): 
        ucur[time_index:time_index+1,:,:] = resample2dToGrid(coverage.read_axis_x(),coverage.read_axis_y(),lon_reg,lat_reg,coverage.read_variable_u_current_at_time_and_level(time,z))
        vcur[time_index:time_index+1,:,:] = resample2dToGrid(coverage.read_axis_x(),coverage.read_axis_y(),lon_reg,lat_reg,coverage.read_variable_v_current_at_time_and_level(time,z))
        time_index += 1  

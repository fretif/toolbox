...
def resample_type_1(coverage,targetResX,targetResY,targetFile=""):
    """Resample HS, DIR, WLV, TAW, TWO and CURRENT """
    ...
    for time in coverage.read_axis_t():  
        logging.info('[Interpolator] Writing variable \'wlv\' at time \''+str(time)+'\'') 
    ...
    #Surface current   
    ucur = ncfile.createVariable('u', float32, ('time', 'latitude', 'longitude',),fill_value="NaN")
    ...
    vcur = ncfile.createVariable('v', float32, ('time', 'latitude', 'longitude',),fill_value="NaN")
    ...
    time_index=0
    for time in coverage.read_axis_t():     
        logging.info('[Interpolator] Writing variable \'current\' at time \''+str(time)+'\'') 
        ucur[time_index:time_index+1,:,:] = resample_2d_to_grid(coverage.read_axis_x(),coverage.read_axis_y(),lon_reg,lat_reg,coverage.read_variable_u_surface_current_at_time(time))
        vcur[time_index:time_index+1,:,:] = resample_2d_to_grid(coverage.read_axis_x(),coverage.read_axis_y(),lon_reg,lat_reg,coverage.read_variable_v_surface_current_at_time(time))
        time_index += 1  

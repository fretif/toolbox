...
# 1. Créer un répertoire dédié aux logiciels & librairies que nous allons installées manuellement. 
mkdir ~/softs
mkdir ~/softs/source

# 2. Installation des entêtes Python & librairies essentielles
sudo apt-get install python-dev liblapack-dev libblas-dev libfreetype6-dev libpng-dev libx11-dev libxaw7-dev m4 patch

# 3. Installation de gfortran & g++ & git
...
# 17. Éditer le fichier ~/.bashrc et insérer ces lignes à la fin du fichier [en remplaçant XXXXXX par votre nom d’utilisateur]
var=$(echo "$wd" | sed 's/\//\\\//g')
cat >> ~/.bashrc << \EOF
#netcdf-4.4.1.1
...
# 18. Installation de la librairie
cd ~
git clone https://github.com/fretif/toolbox.git

# 14. Installation de ‘Pycharm’ version Community
cd ~/softs/source
